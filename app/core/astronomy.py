...
def _geo_kwargs_for_sig(sig: inspect.Signature, *, topocentric: bool, lat_q, lon_q, elev_q) -> Dict[str, Any]:
    ...
    if topocentric:
        ...
        # Some adapters use 'phi'/'lam' or 'lambda'
...
def _normalize_adapter_output_to_maps(
    ...
    if isinstance(res, (list, tuple)):
        # B1: your adapter style → list of dicts with "name","lon","speed"
        if len(res) and isinstance(res[0], dict):
            ...
            for row in res:
                try:
                    ...
                    if not nm:
                        continue
                    # prefer 'lon', but accept 'longitude'/'longitude_deg'
...
def _cached_positions(
    ...
    Accepts many adapter shapes, including your list-of-dicts style.
    """
    if eph is None:
        raise AstronomyError("ephemeris_unavailable", f"ephemeris_adapter import failed: {_EPH_IMPORT_ERROR!r}")

    # Choose a callable (prefer ones likely to exist)
    fn = _adapter_callable(
        "get_ecliptic_longitudes",      # if you added a thin wrapper
        "ecliptic_longitudes",          # your existing function (jd_tt, lat=0.0, lon=0.0) → list[dict]
        "planetary_longitudes",         # alternate names in other adapters
    )
    if fn is None:
    ...
    if "jd_tt" in sig.parameters: kwargs["jd_tt"] = jd_tt_q
        ...
        # Some minimal adapters accept only positional jd_tt
        pass

    # Names (only if accepted by adapter)
    if "names" in sig.parameters:
        kwargs["names"] = list(names_key)

    # Frame kw if available
    if "frame" in sig.parameters:
        kwargs["frame"] = "ecliptic-of-date"

    # Geo/topo kwargs according to adapter param names
    ...
    # Call the adapter
    try:
        res = fn(**kwargs) if kwargs else fn(jd_tt_q)  # your adapter accepts (jd_tt) as sole arg
    except TypeError:
        # Fallback: try just (jd_tt)
        res = fn(jd_tt_q)
...
def _longitudes_and_speeds(
    ...
    *,
    ...
    # If all speeds present from adapter, we're done
    ...
    # Else compute via symmetric FD (uses cached positions)
