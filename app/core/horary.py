...
- Timescales come from compute_chart(...).meta.timescales (jd_tt, jd_ut1).
...
- Public entrypoint: analyze_prasna(method=...), with helpers per system.

Exported API
------------
- HoraryInput, QuestionType
- QuerentBirthData, HybridPrasnaInput
- analyze_prasna (method: "parashari" | "kp" | "hybrid")
...
# (We intentionally DO NOT import app.core.houses.compute_houses)
...
def _houses_from_chart(chart: Dict[str,Any], *, latitude: float, longitude: float,
    ...
    jd_ut = float(ts.get("jd_utc")) if ts else None
    jd_tt = float(ts.get("jd_tt")) if ts else None
    jd_ut1 = float(ts.get("jd_ut1")) if ts else None
...
def analyze_parashari(inp: HoraryInput) -> Dict[str, Any]:
    ...
    d = meta.get("timescales", {}).get("jd_utc")  # just to check presence
    date = inp.date or meta.get("timescales") and "" or datetime.now(timezone.utc).date().isoformat()
