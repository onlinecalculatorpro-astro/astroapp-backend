...
def delta_t_seconds(year: int, month: int) -> float:
    """
    ΔT ≡ TT − UT1 in seconds (Espenak–Meeus style piecewise fit).
    Reasonably accurate across ~1800–2050; gentle extrapolation beyond.
    """
...
# Back-compat alias (some modules use the capital 'T' form)
delta_T_seconds = delta_t_seconds


def jd_tt_from_utc_jd(jd_utc: float, year: int, month: int) -> float:
    """
    Convert JD(UTC) -> JD(TT) using ΔT (TT−UT1).
    NOTE: This does not incorporate DUT1; callers handling UT1 should compute:
...
def civil_timescales(
    ...
    """
    Convenience helper: returns {"jd_utc","jd_tt","jd_ut1","delta_t","dut1"}
    for a local civil instant.
    """
    dt_utc = _to_utc_datetime(date_str, time_str, tz_str)
