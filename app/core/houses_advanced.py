...
def _wrap_diff_deg(a: float, b: float) -> float:
    """Smallest absolute angular difference |a-b| on the circle."""
    d = abs(_norm_deg(a - b))
...
def _placidus(phi: float, eps: float, ramc: float, asc: float, mc: float, *, _diag: Optional[dict] = None) -> List[float]:
    return _solve_time_division_cusps(
        ...
        ramc=ramc, eps=eps, phi=phi, asc=asc, mc=mc, sda_fn=_sda_deg, diag_store=_diag, label_prefix="P:"
    )

def _topocentric_pp(phi: float, eps: float, ramc: float, asc: float, mc: float, *, _diag: Optional[dict] = None) -> List[float]:
    return _solve_time_division_cusps(
        frac_pre_11=1/3, frac_pre_12=2/3, frac_post_2=2/3, frac_post_3=1/3,
        ramc=ramc, eps=eps, phi=phi, asc=asc, mc=mc, sda_fn=_sda_topocentric_deg, diag_store=_diag, label_prefix="PP:"
    )
...
def analyze_interceptions(cusps_deg: List[float]) -> Dict[str, Any]:
    ...
    Returns:
      {
        "intercepted_signs": [{"sign":"Cancer","house":8}, ...],
        "duplicated_signs":  ["Gemini","Sagittarius"],        # signs containing ≥2 cusps
        "house_spans_deg":   [span1, ..., span12],            # forward wrap spans (deg)
        "cusp_signs":        ["Aries", "Taurus", ...],        # sign at each cusp longitude
      }
    """
    if len(cusps_deg) != 12:
        raise ValueError("cusps_deg must have length 12")
...
def analyze_house_sizes(cusps_deg: List[float]) -> Dict[str, Any]:
    """
    Compute per-house spans and summary statistics relative to the 'ideal' 30°.
    Returns:
      {
        "spans_deg": [...12...],
        "mean_deg": float,
        ...
        "min_deg": float,
        "max_deg": float,
        "deviation_deg": [... abs(span-30) ...],
        "total_abs_deviation_deg": float
      }
    """
    if len(cusps_deg) != 12:
        raise ValueError("cusps_deg must have length 12")
...
def gauquelin_sector(ra_deg: float, dec_deg: float, *, ramc_deg: float, phi_deg: float) -> int:
    ...
    NOTE: Scholarly Gauquelin uses semi-arc scaling; this simplified version treats equal hour-angle slices.
    """
...
def run_comprehensive_validation(external_vectors_file: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        'validation_summary': {
            ...
            'max_error_arcsec': max_arc, 'avg_error_arcsec': avg_arc,
            'gold_standard_certified': pass_rate >= 0.95 and max_arc < 36.0
        },
