...
def system_validation():
    ...
    ), 200# ───────────────────────── predictive (transits • validation • dasha • varga • yogas) ─────────────────────────
    ...
    try:
        ...
        for k in list(safe_kwargs.keys()):
            if safe_kwargs[k] is None and k not in ("years_after", "target"):
                safe_kwargs.pop# app/api/routes.py
"""
...
# V2 Prediction Engine Rate Limits
        ...
        preds.append(
            {
                ...
                "horizon": horizon,
                "interval_start_utc":# ───────────────────────── SYNASTRY & COMPOSITE ─────────────────────────
@api.post("/api/synastry")
@rate_limit(RL_SYNASTRY)
def synastry_route():
    """
