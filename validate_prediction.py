# app/core/validate_prediction.py
...
def _parse_tz(s: str, loc: List[str]) -> tzinfo:
    try:
        return ZoneInfo(s)
    except Exception:
        try:
            if s.startswith(("+", "-")):
                ...
                if ":" in rest:
                    ...
                    hh, mm = rest[:2], rest[2:] if len(rest) > 2 else "0"
                offset_minutes = sign * (int(hh) * 60 + int(mm))
                return timezone(timedelta(minutes=offset_minutes))
        except Exception:
            pass
        raise ValidationError({"loc": loc, "msg": "unknown IANA tz or offset like +05:30", "type": "value_error.timezone"})
